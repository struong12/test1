...
%% GUI requests to send an outgoing message Message to chatroom ChatName
	...
	whoami -> 
	    {reselt, self(), Ref, State#cl_st.nick}; %% 3.4
...
do_join(State, Ref, ChatName) ->
    case map:is_key(ChatName, State#cl_st.con_ch) of 
		...
		receive
			{From, Ref, connect, History} ->
				NewState = #cl_st{gui = State#cl_st.gui, nick = State#cl_st.nick, con_ch = maps:put(ChatName, From, State)},
...
do_leave(State, Ref, ChatName) ->
	case map:is_key(ChatName, State#cl_st.con_ch) of	
		false -> whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref, err}, % 2
				{err, State}; 
		true -> whereis(server)!{self(), Ref, leave, ChatName}, % 3
				receive
					{_From, Ref, ack_leave} ->
...
do_msg_send(State, Ref, ChatName, Message) ->
	Chatroom = maps:find(ChatName,State#cl_st.con_ch),	
