...
do_update_nick(State, ClientPID, NewNick) ->
    NewState = #chat_st{name = State#chat_st.name, registrations = maps:update(ClientPID, NewNick, State#chat_st.registrations), history = State#chat_st.history},
    NewState.
...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	lists:foreach(
		fun (CliPID) -> 
			CliPID!{request, self(), Ref, {incoming_msg, CliNick, State#chat_st.name, Message}} 
		end, 
		maps:keys(maps:remove(ClientPID, State#chat_st.registrations))
	),
	NewState = #chat_st{name = State#chat_st.name, registrations = State#chat_st.registrations, history = lists:append(State#chat_st.history, [{CliNick, Message}])},
    NewState.
